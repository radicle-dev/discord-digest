...
export const PROMPTS = {
	...
	detailed: (log: string) => `Summarize the following conversation in detail:\n\n${log}\n\n`,

	important: (log: string) => `${log}\n\nWhat are the important points from this chat log?\n\n`
}

export type PromptStyle = keyof typeof PROMPTS

function buildPrompt(msgs: Collection<string, Message<boolean>>, style: PromptStyle) {
	...
	return {
		messages: promptMsgs.map((m) => m.msg),
...
export const summarizeChannel = async (
	...
	const { messages, prompt } = buildPrompt(msgs, style)
